...
public class MapEditorController {
    ...
    ResponseEntity<MapDTO> createEmptyMap(@Nullable @RequestParam(defaultValue = "10") Integer initialWidth,
        ...
        return ResponseEntity.ok(MapDTO.fromEditor(id, editor));
    }

    @PostMapping("/draw")
    @Description("Draws a tile on a map.")
    ResponseEntity<MapDTO> draw(String editorID, @RequestBody List<MapEditorDrawRequest> drawRequests) {
        ...
        if (editor.isPresent()) {
            ...
            return ResponseEntity.ok(MapDTO.fromEditor(editorID, editor.get()));
    ...
    @PostMapping("/setMap")
    @Description("Draws a tile on a map.")
    ResponseEntity<MapDTO> setMap(String editorID, String map) {
        ...
        if (optional.isPresent()) {
            ...
            return ResponseEntity.ok(MapDTO.fromEditor(editorID, editor));
        }
        return ResponseEntity.notFound().build();
    }
    ...
    ResponseEntity<MapDTO> setFrog(String editorID, @RequestBody FrogDTO frog) {
        Optional<MapEditor> optional = repository.findEditorById(editorID);
        if (optional.isPresent()) {
            MapEditor editor = optional.get();
    ...
    @PostMapping("/setFood")
    @Description("Sets the frog position on the map")
    ResponseEntity<MapDTO> setFood(String editorID, @RequestBody MapEditorFoodRequest req) {
        Optional<MapEditor> optional = repository.findEditorById(editorID);
        if (optional.isPresent()) {
            MapEditor editor = optional.get();
    ...
    ResponseEntity<MapDTO> loadMap(String editorID) {
        Optional<MapEditor> optional = repository.findEditorById(editorID);
        return optional.map(editor -> ResponseEntity.ok(MapDTO.fromEditor(editorID, editor)))
